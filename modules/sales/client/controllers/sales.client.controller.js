(function () {
  ...
  function SalesController (
    ...
    vm.sessionInfo = vm.authentication.session.get('session');
    vm.sucursalInfo = SucursalsService.get({sucursalId: vm.sale.sessionInfo.sucursalId});

    vm.productServices.sucursalSearch = vm.sessionInfo.sucursalId;
  ...
  vm.getProductFilter = function(param){
    ...
    var parm = {
      bardCode: param,
      sucursalId: vm.sessionInfo.sucursalId
    };
    console.log(param);
...
vm.saveOrder = function(order){
  vm.product.sucursalId = vm.sessionInfo.sucursalId;
  ...
  vm.product.caja = vm.sessionInfo.caja;
  vm.product.session = vm.sessionInfo._id;
...
function saveProces(data){
  ...
  if(vm.product.formaPago === 'credito'){
    ...
    if(vm.pay > 0){
     ...
     // vm.printReport();
     vm.inventoryServices.getMaxInventory(vm.sessionInfo.sucursalId)
...
vm.openSalesList = function(){
 vm.salesServices.getSales({session: vm.sessionInfo._id}).then(function(pedingOrder){
