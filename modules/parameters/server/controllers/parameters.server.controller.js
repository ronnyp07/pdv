...
exports.getParamByAncestors = function(req, res) {
    Parameter
    .find({ancestors: {$in : [ "Categoria" ]}}).exec(function(err, Parameter){
      if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.list = function(req, res) {
     ...
     var query = {};
     if(req.query.filter){
       query = {$or: [{systemParam: true, name: {$regex: req.query.filter.name,  $options: '-i'}}, {isActive: true, companyId: req.query.companyId, name: {$regex: req.query.filter.name,  $options: '-i'}}]};
     }else{
       query =  {$or: [{systemParam: true}, {isActive: true, companyId: req.query.companyId}]};
     }
...
exports.getfilterParameter = function(req, res){
    var query = {};
       query =  {$or: [{systemParam: true}, {isActive: true, companyId: req.body.companyId}]};
